...
const zooAnimals = [
 ...
 function animalNames(array){
  let newArr = []
    array.forEach(function (item){return newArr.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`)})
 return newArr
}
 ...
 function lowerCaseNames(arr){
  let lowerCase = arr.map((item) => {return item.animal_name.toLowerCase()})
  return lowerCase
}
 ...
 function lowPopulationAnimals(arr){
  let lowPopulation = arr.filter((item) => {return item.population < 5})
 ...
 function USApop(arr){
  let tPop = arr.reduce(function(total, item){
 ...
 function count(toy, manifacture){
  let count = 0
  for(let i = 0; i < 5; i++){
    count = count + 1;
  }
...
// console.log(cars("Hot wheels",'hasbro'))

function consume(toy, manifacture, cb){
  return `Hey you receved a sale!! You bought a ${toy}, by ${manifacture}. That activated a secret sale!! ${cb(toy, manifacture)} free toys`
}
